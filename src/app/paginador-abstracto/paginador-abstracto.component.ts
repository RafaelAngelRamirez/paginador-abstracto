 ...
 *  Metodo de uso:
 *  
 * Esta es una ``` texto ``` de uso basico para los elementos que tienen que ir 
 * en el html dentro del componente.  
 * ``` html
 * <app-paginador-abstracto
      [debug]="true"
      (actualizacion)="actualizacion($event)"
    >
    </app-paginador-abstracto>
 * ```
 * 
...
@Component({
  ...
  styleUrls: ["./paginador-abstracto.component.css"]
})
export class PaginadorAbstractoComponent implements OnInit {
   ...
   * @memberof PaginadorAbstractoComponent
   */
  elementosPorPagina: number[] = [5, 15, 50]
